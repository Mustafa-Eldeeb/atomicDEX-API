...
impl ChannelOptions {
    #[cfg(not(target_arch = "wasm32"))]
    fn update(&mut self, options: ChannelOptions) {
...
impl OurChannelsConfig {
    #[cfg(not(target_arch = "wasm32"))]
    fn update(&mut self, config: OurChannelsConfig) {
...
pub struct OpenChannelRequest {
    ...
    pub push_msat: u64,
    pub channel_options: Option<ChannelOptions>,
    pub our_channel_config: Option<OurChannelsConfig>,
    pub counterparty_channel_config_limits: Option<CounterpartyLimits>,
}
...
pub async fn open_channel(ctx: MmArc, req: OpenChannelRequest) -> OpenChannelResult<OpenChannelResponse> {
    ...
    if let Some(config) = req.our_channel_config {
        match conf.our_channels_config.as_mut() {
            Some(c) => c.update(config),
            None => conf.our_channels_config = Some(config),
        }
    }
    if let Some(limits) = req.counterparty_channel_config_limits {
        match conf.counterparty_channel_config_limits.as_mut() {
            Some(l) => l.update(limits),
            None => conf.counterparty_channel_config_limits = Some(limits),
        }
    }
